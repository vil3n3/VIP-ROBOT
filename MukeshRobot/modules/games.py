...
async def _weekly(client,message):
    ...
    if x is True:
        ...
        return await message.reply_photo(photo=random.choice(photo), caption="🎁 Yᴏᴜ ʜᴀᴠᴇ ᴄʟᴀɪᴍᴇᴅ ʏᴏᴜʀ ᴡᴇᴇᴋʟʏ ʙᴏɴᴜs ᴏғ 50,000 ᴅᴀʟᴄs!\n• ᴛᴏᴛᴀʟ ᴅᴀʟᴄs ✑ `{0:,}` ᴅᴀʟᴄs".format(coins+50000))    
...
async def _bal(client,message):
    ...
    if user.id not in SUPREME_USERS:
        return 
    if not message.reply_to_message:
        return await message.reply_photo(photo=random.choice(photo), caption="Reply to a User")
    ...
    if not await is_player(from_user.id):
        await create_account(from_user.id,from_user.username) 
    if len(message.command) < 2:
        return await message.reply_photo(photo=random.choice(photo), caption="Give Me a Value to set users dalcs")
    dalcs = message.command[1]
    if not dalcs.isdigit():
        return await message.reply_photo(photo=random.choice(photo), caption="The Provided Value is not a Integer.")
    dalcs = abs(int(dalcs))
    await gamesdb.update_one({'user_id' : from_user.id},{'$set' : {'coins' : dalcs }})
    return await message.reply_photo(photo=random.choice(photo), caption=f"Success! Set the Dalcs of user {from_user.mention} to {dalcs}")
